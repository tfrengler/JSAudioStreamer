...
if ( !MediaSource.isTypeSupported('audio/mp4;codecs="mp4a.40.2"') || new RegExp("Edge/\\d").test(navigator.userAgent) )
{
	document.querySelector("body").innerHTML = "<p>Sorry, this app will not work in this browser as it lacks the proper audio-support needed.</p><p>Try a Chrome-based browser instead.</p>";
	abort = true;
}

if (!abort && window.outerWidth < 1025)
{
	document.querySelector("body").innerHTML = "<p>Sorry, your screen is really small meaning you're probably trying to use this on a mobile or a small tablet.<p><p>This app is made for a desktop/laptop sized screen.</p>";
	abort = true;
}
